...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        pepper_ingredient = Ingredient.objects.create(user=self.user, name='Pepper')
        payload = {'ingredients': [{'name': 'Pepper'}]}
        url = detail_url(recipe.id)
        response = self.client.patch(url, payload, format='json')

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn(pepper_ingredient, recipe.ingredients.all())
    ...
    def test_filter_by_tags(self):
        ...
        recipe1 = create_recipe(user=self.user, title='Test1')
        recipe2 = create_recipe(user=self.user, title='Test2')
        tag1 = Tag.objects.create(user=self.user, name='tag1')
        tag2 = Tag.objects.create(user=self.user, name='tag2')
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)
        recipe3 = create_recipe(user=self.user, title='Test3')
...
class ImageUploadTests(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = get_user_model().objects.create(email='user@example.com', password='testpass123')
